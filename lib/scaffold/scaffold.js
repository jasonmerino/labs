...
module.exports = function(grunt) {
    ...
    var fs = require('fs'),
        path = require('path'),
        apppath = grunt.config.get('app.path'),
    ...
    **/
    ...
    function applyTemplate(contents, layer, name) {
        var data = {
            ...
            namespace: 'application.' + layer + '.' + _s.capitalize(name) + _s.capitalize(layer),
            layer: layer,
            clayer: _s.capitalize(layer),
            name: name,
            cname: _s.capitalize(name),
            appfolder: appfolder,
        };
    ...
    **/
    function add(l, n, op) {
        ...
        var destOpt = 'scaffold.options.' + op,
            ...
            subfix = (op == 'test') ? '_' + l + '_test' : '_' + l,
            file = (dest + l + '/' + n + subfix + getExtension(l, op)),
            tpl = buildTplPath(l, op);
        if(!grunt.file.exists(tpl)) {
            grunt.log.warn('Template ' + tpl + ': not found.');
            return false;
        }
    ...
    **/
    function remove(l, n, op) {
        ...
        var destOpt = ('scaffold.options.' + op),
            dest = grunt.template.process(grunt.config.get(destOpt), { layer: l }),
            subfix = (op == 'test') ? '_' + l + '_test' : '_' + l,
            file = (dest + l + '/' + n + subfix + getExtension());
