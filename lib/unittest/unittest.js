...
module.exports = function(grunt) {
    ...
    // Variables
    var apppath = grunt.config.get('app.path'),
        appfolder = grunt.config.get('app.folder'),
        env = require('../../config/env'),
        cfg = grunt.config.get('test.options'),
        testPath = path.resolve(apppath, cfg.path),
        covpath = path.resolve(apppath, appfolder + '-cov/'),
        covReportPath = path.resolve(apppath, cfg.coveragePath),
    ...
    grunt.registerTask('coverage:create', 'Create Coverage Report', function() {
        ...
        var done = this.async();
        var exclFiles = _.map(cfg.coverageExcludeFiles, function(f) { return path.resolve(apppath, appfolder + '/' + f); }, this).join(',');
        var jscovCmd = 'node_modules/jscoverage/bin/jscoverage --no-highlight --exclude ' + exclFiles + ' ' + apppath + '/' + appfolder + ' ' + covpath;
