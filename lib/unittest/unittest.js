...
module.exports = function(grunt) {
    ...
    // Variables
    var apppath = grunt.config.get('apppath'),
        env = require('../../config/env'),
        cfg = grunt.config.get('test.options'),
        testPath = path.resolve(apppath, cfg.path),
        covpath = path.resolve(apppath, 'application-cov/'), // FIXME: Resolve hardcoded 'application' path.
        covReportPath = path.resolve(apppath, cfg.coveragePath) + '/coverage.html',
    ...
    grunt.registerTask('coverage:create', 'Create Coverage Report', function() {
        ...
        var exclFiles = _.map(cfg.coverageExcludeFiles, function(f) { return path.resolve(apppath, 'application/' + f); }, this).join(',');
        var jscovCmd = 'node_modules/jscoverage/bin/jscoverage --no-highlight ' + apppath + '/application ' + covpath + ' --exclude ' + exclFiles;
    ...
    grunt.registerTask('test:unit:html', 'Execute Mocha Unit tests (Integration with JSCoverage)', function() {
        if(testFiles.length > 0) {
            var done = this.async();
            var mochaCmd = 'export UT=1; node_modules/mocha/bin/mocha -R html-cov -c -t 5000 -u bdd ' + testFiles.join(' ') + " --coverage > " + covReportPath;
            exec(mochaCmd, _.bind(function(error, stdout, stderr) {
                if(error) grunt.fail.fatal(error);
